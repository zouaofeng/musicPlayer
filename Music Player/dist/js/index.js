;(function($, player){
    function MusicPlayer(dom){
        ...
        this.list = null;       //列表切歌对象（在listPlay方法里赋了值）
    ...
    MusicPlayer.prototype = {
        ...
        getDom: function(){ //获取页面中的元素
            ...
            this.controlBtns = document.querySelectorAll('.control li');    //底部导航栏的按钮
        },
        getData: function(url){
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    This.listPlay();        //列表切歌，它要放在loadMusic的前面，它需要在loadMusic里使用
        ...
        loadMusic:function(index){       //加载音乐
            ...
            //播放音乐(只有音乐状态为play的时候才播放)
            if(player.music.status == 'play'){
                player.music.play();
                this.controlBtns[2].className = 'playing';   //把歌曲按钮变成播放状态
        ...
        musicControl: function(){       //音乐控制
            ...
            this.controlBtns[2].addEventListener('touchend',function(){
                if(player.music.status == 'play'){  //歌曲的状态为播放
                    ...
                    //第二次播放需要加上上次播放的角度，但第一次播放时取不到，需要容错处理
        ...
        imgRotate: function(deg){ //旋转唱片
            ...
            this.rotateTimer = setInterval(function(){
                deg = +deg + 0.2;   //前面的加号将字符串转成数字

                This.record.style.transform = 'rotate(' + deg + 'deg)';
                This.record.dataset.rotate = deg; //把旋转的角度存到标签身上，为了暂停后继续播放嫩取到
            }, 1000/60)
        },
        ...
        listPlay: function(){             //列表切歌
            var This = this;
            this.list = player.listControl(this.dataList, this.wrap);  //把listControl对象赋值给list
            ...
            this.list.musicList.forEach(function(item, index){
                item.addEventListener('touchend', function(){
                    ...
                    This.indexObj.index = index;    //索引值对象上的索引值要更新
                    This.loadMusic(index);          //加载点击对应的索引值的歌曲
